# ProspectSA — Standalone Setup (No Replit Required)

This guide removes ALL Replit-specific dependencies so the app runs on any
machine: local dev, VPS, cloud VM, or Docker.

---

## What needs to change to remove Replit

| Replit thing | What it does | Standalone replacement |
|---|---|---|
| `AI_INTEGRATIONS_OPENAI_API_KEY` | Proxy key for OpenAI | Use direct `OPENAI_API_KEY` ✅ already set |
| `AI_INTEGRATIONS_ANTHROPIC_*` | Proxy key for Anthropic | Use direct `ANTHROPIC_API_KEY` |
| `DATABASE_URL` | Auto-provided PostgreSQL | Your own PostgreSQL instance |
| `replit` config file | Defines workflow tasks | Use the commands below instead |
| `modules.yaml` | Nix package list | Install via your OS package manager |
| `@replit/vite-plugin-*` | Dev banner + error overlay | Remove from `vite.config.ts` (optional) |
| Replit secrets tab | Env var storage | Your `.env` file (already created) |
| PORT auto-assignment | Picks a free port | Set `PORT=3000` in `.env` |

**The code itself has no Replit-specific logic.** `lib/config/env.ts` checks for
`AI_INTEGRATIONS_*` vars and falls back to direct keys automatically. You just
need your own keys and a PostgreSQL database.
...
## Optional: Remove Replit-specific Vite plugins

The frontend `vite.config.ts` includes Replit dev plugins that cause warnings
outside Replit. To remove them, edit `artifacts/prospect-sa/vite.config.ts`:
...
// cartographer(), devBanner(), runtimeErrorModal()
```

The app runs fine without them — they're just Replit dev conveniences.
