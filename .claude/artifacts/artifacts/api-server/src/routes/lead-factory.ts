...
router.post("/api/lead-factory/start", async (req: Request, res: Response) => {
...
router.get("/api/lead-factory/stream/:jobId", (req: Request, res: Response) => {
  const emitter = getLeadFactoryEmitter(req.params.jobId);
  if (!emitter) {
...
router.get("/api/lead-factory/jobs", async (_req: Request, res: Response) => {
  try {
...
router.get("/api/lead-factory/jobs/:jobId", async (req: Request, res: Response) => {
  try {
...
// POST /api/lead-factory/results/:jobId/publish
...
router.post("/api/lead-factory/results/:jobId/publish", async (req: Request, res: Response) => {
  try {
    const id = parseInt(req.params.jobId, 10);
...
router.post("/api/lead-factory/jobs/:jobId/cancel", (req: Request, res: Response) => {
  const ok = cancelLeadFactoryJob(req.params.jobId);
...
router.post("/api/relationship-intel/jobs/:jobId/cancel", (req: Request, res: Response) => {
  const ok = cancelRelationshipIntelJob(req.params.jobId);
  if (!ok) return res.status(404).json({ ok: false, error: "Job not found or already finished" });
  res.json({ ok: true });
});

// POST /api/signals/jobs/:jobId/cancel
router.post("/api/signals/jobs/:jobId/cancel", (req: Request, res: Response) => {
  const ok = cancelSignalJob(req.params.jobId);
  if (!ok) return res.status(404).json({ ok: false, error: "Job not found or already finished" });
  res.json({ ok: true });
});

// GET /api/lead-factory/results/:jobId
router.get("/api/lead-factory/results/:jobId", async (req: Request, res: Response) => {
  try {
...
router.post("/api/relationship-intel/start", async (req: Request, res: Response) => {
  try {
...
// GET /api/relationship-intel/stream/:jobId (SSE)
router.get("/api/relationship-intel/stream/:jobId", (req: Request, res: Response) => {
  const emitter = getRelationshipIntelEmitter(req.params.jobId);
  if (!emitter) {
    res.status(404).json({ ok: false, error: "Job not found" });
    return;
  }
...
// GET /api/relationship-intel/jobs/:jobId
router.get("/api/relationship-intel/jobs/:jobId", async (req: Request, res: Response) => {
  try {
    const id = parseInt(req.params.jobId, 10);
...
// GET /api/relationship-intel/jobs
router.get("/api/relationship-intel/jobs", async (_req: Request, res: Response) => {
  try {
...
// GET /api/lead-factory/company-suggest?q=aramco
router.get("/api/lead-factory/company-suggest", async (req: Request, res: Response) => {
...
// POST /api/signals/push  — starts a signal monitor run, returns SSE jobId
router.post("/api/signals/push", async (req: Request, res: Response) => {
  try {
...
// GET /api/signals/stream/:jobId (SSE)
router.get("/api/signals/stream/:jobId", (req: Request, res: Response) => {
  const emitter = getSignalEmitter(req.params.jobId);
  if (!emitter) {
    res.status(404).json({ ok: false, error: "Job not found" });
    return;
  }
