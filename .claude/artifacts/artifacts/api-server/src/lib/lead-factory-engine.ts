...
export const leadFactoryBriefSchema = z.object({
  ...
  enrichmentDepth: z.enum(["basic", "standard", "deep"]).optional(),
  country: z.string().optional(),
  autoEnrichDownstream: z.boolean().optional(),
}).refine(
  (b) => b.inputMode !== "list" || (b.companyList && b.companyList.length > 0),
  { message: "companyList is required when inputMode='list'" },
);
  ...
  for (let i = 0; i < leads.length; i++) {
    ...
    if (lead.domain) {
      try {
        ...
        if (scout.ok) {
          ...
          if (scout.emails?.[0] && !lead.emailTrusted) {
            lead.email = scout.emails[0];
            lead.emailTrusted = true;
          } else {
            lead.email = lead.email || scout.emails?.[0];
            if (scout.emails?.[0]) lead.emailTrusted = true;
          }
          lead.phone = lead.phone || scout.phones?.[0];
          lead.linkedinUrl = lead.linkedinUrl || scout.socials?.linkedin;
          lead.crNumber = lead.crNumber || scout.cr_vat?.cr_number;
          lead.description = lead.description || scout.meta?.description;
          lead.logoUrl = lead.logoUrl || scout.meta?.og_image;
          lead.enrichedData = { ...lead.enrichedData, techStack: scout.tech_stack, socials: scout.socials };
    ...
    if (depth === "deep" && lead.domain && !lead.email) {
      try {
        ...
        if (scraped.ok && scraped.text && scraped.text.length > 100) {
  ...
  for (let i = 0; i < leads.length; i++) {
    ...
    try {
      ...
      const [fullSig, regSig] = await Promise.allSettled([
        scoutSignalsFull(name, lead.crNumber),
        scoutSignalsRegulatory(name, lead.crNumber),
      ]);

      if (fullSig.status === "fulfilled" && fullSig.value?.ok) {
        lead.signalData = { ...lead.signalData, full: fullSig.value };
        const d = fullSig.value.data || {};
        const flags = d.red_flags_count || 0;
        if (flags > 0) emit(emitter, { type: "agent_log", agent: 4, message: `⚠ ${name}: ${flags} compliance flag(s)` });
      }

      if (regSig.status === "fulfilled" && regSig.value?.ok) {
        lead.signalData = { ...lead.signalData, regulatory: regSig.value };
      }

      // Buying signal classification
      const buyingSignals: string[] = [];
      const sig = (lead.signalData?.full?.data || {}) as Record<string, unknown>;
      ...
      if ((sig.intent_signals as Record<string, unknown>)?.score > 60) buyingSignals.push("intent");
...
async function agent6_scoreAndCopywrite(
  leads: Array<RawLead & { status: "pass" | "warn"; reasons: string[]; isDuplicate: boolean; duplicateOf?: string }>,
  brief: LeadFactoryBrief,
  emitter: EventEmitter,
): Promise<ScoredLead[]> {
  ...
  for (let i = 0; i < leads.length; i++) {
    ...
    const fullData = (lead.signalData as Record<string, unknown>)?.full?.data as Record<string, unknown>;
    let riskScore = 0;
    if ((regData?.summary as Record<string, unknown>)?.total_flags > 0) riskScore += 40;
...
async function agent7_publishAndSeed(
  ...
  for (let i = 0; i < leads.length; i++) {
    ...
    try {
      ...
      let publishedCompanyId: number | undefined;
 ...
 * was available.
 */
async function upsertCompanyFromLead(lead: ScoredLead): Promise<number | undefined> {
  ...
  if (!existing && cr) {
    const rows = await db.select({ id: companiesTable.id })
      .from(companiesTable)
      .where(eq(companiesTable.crNumber, cr))
      .limit(1);
    existing = rows[0];
  }
  if (!existing && name) {
    const rows = await db.select({ id: companiesTable.id })
      .from(companiesTable)
      .where(ilike(companiesTable.nameEn, name))
      .limit(1);
    existing = rows[0];
  }
  ...
  const values = {
    nameEn: name || null,
    nameAr: lead.companyNameAr || null,
    ...
    foundingYear: lead.foundingYear ? parseInt(lead.foundingYear, 10) || null : null,
    crNumber: cr || null,
    entityType: lead.entityType || null,
    logoUrl: lead.logoUrl || null,
    linkedinUrl: lead.linkedinUrl || null,
    enrichmentScore: lead.icpScore ?? null,
    enrichmentStatus: lead.enrichedData ? "enriched" : "pending",
    dataSource: "lead-factory",
  };

  if (existing) {
    // Update only fields we now have that aren't null; never overwrite with null
  ...
  const [row] = await db.insert(companiesTable).values(values).returning({ id: companiesTable.id });
  return row.id;
}
 ...
 */
async function upsertExecutivesFromLead(lead: ScoredLead, companyId: number): Promise<void> {
  ...
  for (const exec of raw) {
    ...
    const existing = await db.select({ id: executivesTable.id })
      ...
      .where(and(eq(executivesTable.companyId, companyId), ilike(executivesTable.name, name)))
      .limit(1);
...
export async function runLeadFactoryPipeline(
  ...
  try {
    ...
    emit(emitter, {
      ...
      totalSeededToPool: publishedCompanies.length,
