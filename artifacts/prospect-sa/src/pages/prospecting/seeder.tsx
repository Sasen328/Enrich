        ...
        return (
          <div key={label} className="flex items-center gap-1.5 flex-1">
            <div className={`w-6 h-6 rounded-full flex items-center justify-center text-xs font-bold shrink-0 transition-all
              ${done ? "bg-amber-500 text-white" : active ? "bg-amber-500/20 text-amber-400 border border-amber-500/50" : "bg-white/5 text-muted-foreground border border-white/10"}`}>
...
function RecordCard({ record, idx }: { record: Record<string, string>; idx: number }) {
  ...
  return (
    <Card className="bg-card/50 border-white/8 overflow-hidden">
      ...
      {open && (
        <div className="border-t border-white/6 px-4 pb-4 pt-3">
          <div className="grid grid-cols-2 gap-2 text-xs">
            {phone && <div className="flex items-center gap-1.5 text-white/70"><Phone className="w-3 h-3 text-amber-400 shrink-0" />{phone}</div>}
            {website && <div className="flex items-center gap-1.5 text-white/70"><ExternalLink className="w-3 h-3 text-amber-400 shrink-0" /><a href={website.startsWith("http") ? website : `https://${website}`} target="_blank" rel="noopener noreferrer" className="hover:text-white truncate">{website}</a></div>}
            {address && <div className="flex items-start gap-1.5 text-white/70 col-span-2"><MapPin className="w-3 h-3 text-amber-400 shrink-0 mt-0.5" />{address}</div>}
          ...
          {desc && <p className="text-xs text-white/60 mt-2 leading-relaxed">{desc}</p>}
        </div>
      )}
    </Card>
  );
    ...
    <div className="p-6 max-w-2xl mx-auto">
      <div className="flex items-center gap-3 mb-6">
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-xl font-display font-bold text-white">Data Seeder</h1>
            <p className="text-xs text-muted-foreground">
      ...
      <Card className="bg-card/60 border-white/8">
        <CardContent className="p-6">
          ...
          {((form.inputType === "text" && step === 1) || (form.inputType === "url" && urlStep === 1)) && (
            <div className="flex gap-1 p-1 bg-white/5 rounded-xl border border-white/8 w-fit mb-5">
              <button onClick={() => { set("inputType", "text"); setStep(1); }}
                className={`flex items-center gap-2 px-4 py-2 rounded-lg text-sm font-medium transition-all ${form.inputType === "text" ? "bg-amber-500/20 text-white border border-amber-500/30" : "text-muted-foreground hover:text-white"}`}>
                <FileText className="w-4 h-4" />Describe in text
              </button>
              <button onClick={() => { set("inputType", "url"); setUrlStep(1); }}
                className={`flex items-center gap-2 px-4 py-2 rounded-lg text-sm font-medium transition-all ${form.inputType === "url" ? "bg-amber-500/20 text-white border border-amber-500/30" : "text-muted-foreground hover:text-white"}`}>
          ...
          {form.inputType === "text" && step === 1 && (
            <div className="space-y-4">
              <div>
                <p className="text-base font-semibold text-white mb-1">What data do you want to seed?</p>
              ...
              <div>
                ...
                <Textarea
                  ...
                  className="bg-white/5 border-white/10 text-white placeholder:text-muted-foreground/60 min-h-[120px] text-sm resize-none"
                />
              </div>
          ...
          {form.inputType === "text" && step === 2 && (
            <div className="space-y-5">
              <div>
                <p className="text-base font-semibold text-white mb-1">Refine your request</p>
                ...
                <div className="flex flex-wrap gap-1.5">
                  {INDUSTRIES.map(ind => (
                    <button key={ind} onClick={() => set("industry", ind)}
                      className={`text-xs px-3 py-1.5 rounded-lg border transition-all ${form.industry === ind ? "border-amber-500/40 bg-amber-500/10 text-white" : "border-white/8 text-muted-foreground hover:border-white/20"}`}>
                      {ind}
                    </button>
                  ))}
                </div>
                ...
                <div className="flex flex-wrap gap-1.5">
                  {CITIES.map(c => (
                    <button key={c} onClick={() => set("city", c)}
                      className={`text-xs px-3 py-1.5 rounded-lg border transition-all ${form.city === c ? "border-amber-500/40 bg-amber-500/10 text-white" : "border-white/8 text-muted-foreground hover:border-white/20"}`}>
                      {c}
                    </button>
                  ))}
                </div>
                ...
                <div className="flex gap-2">
                  {COUNT_OPTIONS.map(n => (
                    <button key={n} onClick={() => set("count", n)}
                      className={`px-5 py-2 rounded-lg border text-sm font-medium transition-all ${form.count === n ? "border-amber-500/40 bg-amber-500/10 text-white" : "border-white/8 text-muted-foreground hover:border-white/20"}`}>
                      {n}
                    </button>
                  ))}
                </div>
          ...
          {form.inputType === "text" && step === 3 && (
            <div className="space-y-4">
              <div>
                <p className="text-base font-semibold text-white mb-1">Ready to generate</p>
              ...
              <div className="space-y-2">
                {form.prompt && (
                  <div className="p-3 rounded-xl bg-white/5 border border-white/8">
                    ...
                    <p className="text-sm text-white">{form.prompt}</p>
                  </div>
                )}
                ...
                {form.extraContext && (
                  <div className="p-3 rounded-xl bg-white/5 border border-white/8">
                    ...
                    <p className="text-xs text-white/80">{form.extraContext}</p>
                  </div>
                )}
              </div>
          ...
          {form.inputType === "url" && urlStep === 1 && (
            <div className="space-y-4">
              <div>
                <p className="text-base font-semibold text-white mb-1">What URL contains companies?</p>
              ...
              <div className="p-3 rounded-xl bg-white/3 border border-white/8 text-xs text-muted-foreground space-y-1.5">
                <p className="font-medium text-white/60">Powered by 7 parallel agents:</p>
            ...
            <div className="space-y-5">
              <div>
                <div className="flex items-start gap-3 mb-4">
                  ...
                  <div>
                    <p className="text-sm font-semibold text-white">{urlAnalysis.siteType}</p>
                ...
                <p className="text-base font-semibold text-white mb-1">Extraction questionnaire</p>
              ...
              <div className="space-y-4">
                {urlAnalysis.questions.map((q) => (
                  <div key={q.id} className="space-y-2">
                    <label className="text-sm text-white font-medium">{q.question}</label>
                    {q.type === "choice" && q.options && (
                      <div className="flex flex-wrap gap-2">
                        {q.options.map(opt => (
                          <button key={opt} onClick={() => setUrlAnswers(a => ({ ...a, [q.id]: opt }))}
                            className={`text-xs px-3 py-1.5 rounded-lg border transition-all ${urlAnswers[q.id] === opt ? "border-amber-500/40 bg-amber-500/10 text-white" : "border-white/8 text-muted-foreground hover:border-white/20"}`}>
                            {opt}
                          </button>
                        ))}
                      </div>
                    ...
                    {q.type === "boolean" && (
                      <div className="flex gap-2">
                        {["Yes", "No"].map(opt => (
                          <button key={opt} onClick={() => setUrlAnswers(a => ({ ...a, [q.id]: opt }))}
                            className={`text-xs px-4 py-1.5 rounded-lg border transition-all ${urlAnswers[q.id] === opt ? "border-amber-500/40 bg-amber-500/10 text-white" : "border-white/8 text-muted-foreground hover:border-white/20"}`}>
                            {opt}
                          </button>
                        ))}
                      </div>
          ...
          {form.inputType === "url" && urlStep === 3 && (
            <div className="space-y-4">
              <div>
                <p className="text-base font-semibold text-white mb-1">Ready to extract</p>
              ...
              <div className="space-y-2">
                ...
                {urlDescription && (
                  <div className="p-3 rounded-xl bg-white/5 border border-white/8">
                    ...
                    <p className="text-xs text-white/80">{urlDescription}</p>
                  </div>
                )}
                {Object.entries(urlAnswers).filter(([, v]) => v).length > 0 && (
                  <div className="p-3 rounded-xl bg-white/5 border border-white/8">
                    ...
                    {Object.entries(urlAnswers).filter(([, v]) => v).map(([k, v]) => (
                      <p key={k} className="text-xs text-white/70">{k}: <span className="text-white">{v}</span></p>
                    ))}
                  </div>
                )}
              </div>
            ...
            <div className="flex gap-3 mt-6">
              {step < 3 && (
                <>
                  ...
                  <Button onClick={() => setStep(s => s + 1)} className="flex-1 bg-amber-600 hover:bg-amber-700 text-white">
              ...
              {step === 3 && (
                <>
                  <Button variant="outline" className="border-white/10 text-white hover:bg-white/5" onClick={() => setStep(2)}>
                    <ArrowLeft className="w-4 h-4 mr-1.5" />Back
                  </Button>
          ...
          {form.inputType === "url" && (
            <div className="flex gap-3 mt-6">
              ...
              {urlStep === 2 && (
                <>
                  <Button variant="outline" className="border-white/10 text-white hover:bg-white/5" onClick={() => setUrlStep(1)}>
                    <ArrowLeft className="w-4 h-4 mr-1.5" />Back
                  </Button>
                  <Button onClick={() => setUrlStep(3)} className="flex-1 bg-amber-600 hover:bg-amber-700 text-white">
                    Continue<ArrowRight className="w-4 h-4 ml-1.5" />
                  </Button>
                </>
              )}
              {urlStep === 3 && (
                <>
                  <Button variant="outline" className="border-white/10 text-white hover:bg-white/5" onClick={() => setUrlStep(2)}>
                    <ArrowLeft className="w-4 h-4 mr-1.5" />Back
                  </Button>
                  <Button
