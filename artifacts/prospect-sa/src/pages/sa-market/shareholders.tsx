        ...
        <SheetHeader className="px-6 py-5 bg-gradient-to-r from-primary/15 to-violet-500/10 border-b border-border/40 sticky top-0 z-10 backdrop-blur-sm">
          <div className="flex items-center gap-4">
            ...
            <div className="flex-1 min-w-0">
              <SheetTitle className="text-foreground font-display font-bold text-lg leading-tight">{person.name}</SheetTitle>
        ...
        <div className="px-6 py-5 space-y-5">
          ...
          {generateMutation.isPending && (
            <div className="flex flex-col items-center gap-3 py-10">
              ...
              <p className="text-foreground font-medium">AI is researching {person.name}…</p>
            ...
            <>
                ...
                <div className="grid grid-cols-1 gap-2">
                  ...
                  <div className="grid grid-cols-2 gap-2">
                    {profile.investmentAppetite && (
                      <div className="p-3 bg-muted/40 rounded-xl border border-border/40">
                        ...
                        <Badge className={`text-xs border ${APPETITE_COLORS[profile.investmentAppetite] ?? "bg-zinc-500/15 text-muted-foreground"}`}>{profile.investmentAppetite}</Badge>
                      </div>
                    )}
                    {profile.riskProfile && (
                      <div className="p-3 bg-muted/40 rounded-xl border border-border/40">
                        <p className="text-xs text-muted-foreground mb-1">Risk Profile</p>
                        <Badge className={`text-xs border ${RISK_COLORS[profile.riskProfile] ?? "bg-zinc-500/15 text-muted-foreground"}`}>{profile.riskProfile}</Badge>
                      </div>
                    )}
                  </div>
                  {profile.investmentFocus && (
                    <div className="p-3 bg-muted/40 rounded-xl border border-border/40">
                      ...
                      <p className="text-sm text-foreground">{profile.investmentFocus}</p>
                    </div>
                  )}
                </div>
              </div>
              ...
              <div className="space-y-3">
                ...
                {profile.educationBackground && (
                  <div className="p-3 bg-muted/40 rounded-xl border border-border/40">
                    ...
                    <p className="text-sm text-foreground">{profile.educationBackground}</p>
                  </div>
                )}
                {profile.careerHistory && (
                  <div className="p-3 bg-muted/40 rounded-xl border border-border/40">
                    <p className="text-xs text-muted-foreground mb-0.5">Career History</p>
                    <p className="text-sm text-foreground leading-relaxed">{profile.careerHistory}</p>
                  </div>
                )}
                {profile.boardMemberships && (
                  <div className="p-3 bg-muted/40 rounded-xl border border-border/40">
                    <p className="text-xs text-muted-foreground mb-0.5">Board & Memberships</p>
                    <p className="text-sm text-foreground">{profile.boardMemberships}</p>
                  </div>
                )}
              </div>
              ...
              {profile.keyConnections && (
                <div className="space-y-2">
                  ...
                  <div className="p-3 bg-muted/40 rounded-xl border border-border/40">
                    <p className="text-sm text-foreground">{profile.keyConnections}</p>
                  </div>
                </div>
              )}
              ...
              <div className="space-y-3">
                ...
                <div className="grid grid-cols-2 gap-2">
                  {profile.philanthropyInterests && (
                    <div className="p-3 bg-muted/40 rounded-xl border border-border/40">
                      ...
                      <p className="text-xs text-foreground">{profile.philanthropyInterests}</p>
                    </div>
                  )}
                  {profile.geographicPresence && (
                    <div className="p-3 bg-muted/40 rounded-xl border border-border/40">
                      <p className="text-xs text-muted-foreground mb-0.5">Geography</p>
                      <p className="text-xs text-foreground">{profile.geographicPresence}</p>
                    </div>
                  )}
                </div>
                {profile.languagesSpoken && (
                  <div className="p-3 bg-muted/40 rounded-xl border border-border/40">
                    ...
                    <p className="text-sm text-foreground">{profile.languagesSpoken}</p>
                  </div>
                )}
              </div>
...
function ShareholderCard({ sh, onProfile }: { sh: Shareholder; onProfile: (sh: Shareholder) => void }) {
  ...
  return (
    <div className="flex items-center gap-3 py-2.5 px-4 rounded-xl hover:bg-muted/40 transition-colors group">
      ...
      <div className="flex-1 min-w-0">
        <p className="text-sm font-medium text-foreground truncate">{sh.shareholderName}</p>
          ...
          <div className="flex items-start gap-3">
            ...
            <div className="flex-1 min-w-0">
              <div className="flex items-center gap-2 flex-wrap">
                <h3 className="font-display font-semibold text-foreground text-sm leading-tight">{company.companyName}</h3>
            ...
            <div className="flex flex-col items-end gap-1 shrink-0">
              <div className="flex items-center gap-1">
                ...
                <span className="text-sm font-semibold text-foreground">{company.shareholderCount}</span>
...
export default function ShareholdersPage() {
  ...
  return (
    <div className="space-y-5 animate-in slide-in-from-bottom-4 duration-500">
        ...
        <CardContent className="py-3 px-4">
          <div className="flex flex-wrap gap-3 items-center">
            ...
            <div className="flex items-center gap-2 shrink-0">
              ...
              <span className="text-xs font-semibold text-foreground w-8">{minOwnership}%</span>
            </div>
      ...
      {isLoading ? (
        <div className="space-y-3">
          {Array.from({ length: 6 }).map((_, i) => <div key={i} className="h-24 bg-muted/40 rounded-xl animate-pulse" />)}
        </div>
      ) : viewMode === "grouped" ? (
        <div className="space-y-3">
        ...
        <div className="space-y-1">
          {(rows as Shareholder[]).map(sh => (
            <Card key={sh.id} className="bg-card/65 border-border/40 hover:border-white/20 transition-all">
      ...
      {rows.length === 0 && !isLoading && (
        <Card className="bg-card/30 border-border/40 border-dashed">
          <CardContent className="py-12 text-center">
            ...
            <p className="text-foreground font-medium">No shareholders found</p>
